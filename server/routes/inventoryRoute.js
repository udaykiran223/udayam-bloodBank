...
router.post('/add', authMiddleware, async (req, res) => {
    try {
        ...
        if (req.body.inventoryType === "in" && user.userType !== "donar") {
            throw new Error("This Email is not recognized as a donar");
        }
        if (req.body.inventoryType === "out" && user.userType !== "hospital") {
            throw new Error("This Email is not recognized as a hospital");
        }
...
router.post('/filter', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const inventory = await Inventory.find(
            req.body.filters
        ).limit(req.body.limit || 10).sort({ createdAt: -1 }).populate("donar").populate("hospital").populate("organization");
        return res.send({ success: true, data: inventory });
    } catch (error) {
        return res.send({
            success: false,
